...
def load_timeframe_state(inst_id: str, timeframe: str) -> Dict:
    ...
    state_file = VAULT_BASE / 'state' / 'wicks' / 'okx' / \
        'perps' / f'{inst_id}.{timeframe}.state.json'
...
def load_existing_wick_events(inst_id: str) -> Dict[str, WickEvent]:
    ...
    events_file = VAULT_BASE / 'derived' / 'wicks' / \
        'okx' / 'perps' / inst_id / 'wicks_events.jsonl'
...
def write_wicks_state(inst_id: str, wicks: Dict[str, WickEvent]):
    ...
    state_file = VAULT_BASE / 'derived' / 'wicks' / \
        'okx' / 'perps' / inst_id / 'wicks_state.json'
    state_file.parent.mkdir(parents=True, exist_ok=True)
...
def check_wick_touch(wick: WickEvent, future_candle: Candle, tick_sz: Optional[Decimal]) -> Optional[Dict]:
    ...
    if touch_by_wick and tick_sz is not None and tick_sz > 0:
        ...
        tip_near = (tip_distance_ticks >=
                    1 and tip_distance_ticks <= wick.tol_tip_ticks)
        ...
        if wick.wick_type == 'high':
            penetration_ticks = int(
                max(Decimal('0'), extremum - wick_price) / tick_sz)
        else:  # 'low'
            penetration_ticks = int(
                max(Decimal('0'), wick_price - extremum) / tick_sz)

    # Calculate age at touch
    creation_time = datetime.fromisoformat(
        wick.creation_time_utc.replace('Z', '+00:00'))
    touch_time = datetime.fromisoformat(
        future_candle.window_end_utc.replace('Z', '+00:00'))
...
def process_inst_id(inst_id: str):
    ...
    try:
        ...
        if tick_sz_str and tick_sz_str != '':
            try:
                ...
                logger.warning(
                    f"Invalid tickSz: {tick_sz_str}, tip metrics will be null")
        else:
            logger.warning(
                f"No tickSz for {inst_id}, tip metrics will be null")
    ...
    # Pre-group wicks by timeframe to avoid O(N) dict comprehension inside the TIMEFRAMES loop
    wicks_by_tf = {t: {} for t in TIMEFRAMES}
    for eid, w in all_wicks.items():
        if w.timeframe in wicks_by_tf:
            wicks_by_tf[w.timeframe][eid] = w
    ...
    for tf in TIMEFRAMES:
        ...
        if last_processed:
            new_candles = [
                c for c in candles_sorted if c.window_end_utc > last_processed]
            logger.info(
                f"Incremental: {len(new_candles)} new candles (last processed: {last_processed})")
        else:
            new_candles = candles_sorted
            logger.info(
                f"First run: processing all {len(new_candles)} candles")
        ...
        as_of_utc = datetime.fromisoformat(
            new_candles[-1].window_end_utc.replace('Z', '+00:00'))
        ...
        logger.info(
            f"{tf} complete: {new_wicks_count} new, {updated_wicks_count} updated, {expired_wicks_count} expired")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Untouched Wick Tracker (PATCHED)')
    parser.add_argument('--instId', required=True,
    ...
    if args.instId not in INSTRUMENTS:
        logger.error(
            f"BUILD_FAIL: Invalid instId: {args.instId} (allowed: {INSTRUMENTS})")
        return
